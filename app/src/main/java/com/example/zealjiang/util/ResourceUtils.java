...
public class ResourceUtils {
    ...
    public static boolean copyFileFromAssets(final String assetsFilePath, final String destFilePath) {
        boolean res = true;
        try {
            String[] assets = MyApplication.getContext().getAssets().list(assetsFilePath);
            if (assets.length > 0) {
                for (String asset : assets) {
                    //Log.d("mtest","copyFileFromAssets destFilePath: "+destFilePath + "/" + asset);
                    res &= copyFileFromAssets(assetsFilePath + "/" + asset, destFilePath + "/" + asset);
                }
            } else {
                //Log.d("mtest","copyFileFromAssets writeFileFromIS destFilePath: "+destFilePath);
                ...
                if(file != null && file.exists()){
                    ...
                    res = writeFileFromIS(
                            destFilePath,
                            MyApplication.getContext().getAssets().open(assetsFilePath),
                            false
                    );
                }
    ...
    public static String readAssets2String(final String assetsFilePath, final String charsetName) {
        InputStream is;
        try {
            is = MyApplication.getContext().getAssets().open(assetsFilePath);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static List<String> readAssets2List(final String assetsPath,
        ...
        try {
            return is2List(MyApplication.getContext().getResources().getAssets().open(assetsPath), charsetName);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static boolean copyFileFromRaw(@RawRes final int resId, final String destFilePath) {
        return writeFileFromIS(
                destFilePath,
                MyApplication.getContext().getResources().openRawResource(resId),
                false
        );
    }
    ...
    public static String readRaw2String(@RawRes final int resId, final String charsetName) {
        InputStream is = MyApplication.getContext().getResources().openRawResource(resId);
        byte[] bytes = is2Bytes(is);
        if (bytes == null) return null;
        if (isSpace(charsetName)) {
            return new String(bytes);
    ...
    public static String readStrResToStr(@StringRes int resId) {
        String result = "";
        try{
    ...
    public static List<String> readRaw2List(@RawRes final int resId,
                                            final String charsetName) {
        return is2List(MyApplication.getContext().getResources().openRawResource(resId), charsetName);
    }
